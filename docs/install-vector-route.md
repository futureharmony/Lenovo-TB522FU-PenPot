...
| 11 | **笔身触控条手势完全不通**（上滑/下滑/双击无反应），日志 0 条 `mapped/grabbed touch strip` | 两处硬编码不匹配本机：① 设备名串写死 `lenovo tab pen pro consumer control`，本机实名 `Lenovo Tab Pen Pro 2 Consumer Control`（多 ` 2`）→ `contains` 永不成立；② 框架路径按 `keyCode==131/132/133` 分派，而该设备 `getevent -lp` 显示**只支持 `KEY_UNKNOWN`(240)**，真正区分手势的是 `MSC_SCAN` 扫描码 | 名称改为 `contains("lenovo tab pen") && contains("consumer control")`（`SystemStylusHooks.handle` + `LenovoConsumerGestureReader.findNode`）；框架路径改按 `KeyEvent.getScanCode()` 分派。**实测扫描码**（TB522FU，v4.1.5）：`0x000c0613`(787987)=上滑、`0x000c0612`(787986)=下滑、`0x000c0601`(787969)=双击，均随 `KEY_UNKNOWN` DOWN/UP 成对出现，取 UP 触发 |
